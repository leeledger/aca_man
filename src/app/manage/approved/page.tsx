...
export default function ApprovedUsersManagement() {
    ...
    const fetchApprovedUsers = async () => {
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(data.error || '승인된 사용자 목록을 가져오는데 실패했습니다.')
        }
        ...
        if (error instanceof Error) {
          ...
          setError('승인된 사용자 목록을 가져오는데 실패했습니다.')
        }
      } finally {
        setIsLoading(false)
      }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold">승인된 회원 목록</h1>
        <Link 
      ...
      {approvedUsers.length === 0 ? (
        <div className="bg-gray-50 p-6 rounded-lg border border-gray-200 text-center">
          <p className="text-gray-600">승인된 회원이 없습니다.</p>
        </div>
      ) : (
