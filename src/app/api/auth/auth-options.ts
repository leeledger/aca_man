...
// Define authOptions and export it for use in other files
export const authOptions: NextAuthOptions = {
  ...
  callbacks: {
        ...
        try {
          ...
          if (!dbUser) {
            ...
            dbUser = await prisma.user.create({
              data: {
                name: (profile as any)?.kakao_account?.profile?.nickname || '카카오 사용자',
                email: kakaoEmail,
                role: 'TEACHER', // 기본 역할
    ...
    async jwt({ token, user }) {
      if (user) {
        token.id = user.id
        token.role = user.role as 'SUPER_ADMIN' | 'ADMIN' | 'TEACHER'
